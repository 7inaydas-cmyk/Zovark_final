# schema-migration-integrity Specification

## Purpose

Guarantee that the `schema_migrations` ledger accurately reflects the physical PostgreSQL schema, and that Stage 5 Store dependencies are present before any investigation can complete. Define the contract for repairing ledger drift in an idempotent, auditable, and verifiable way.
